...
class Shelf:
    ...
    def populate_books(self, excel_file):
        ...
        logging.info(f"Data entries from {excel_file} added to the shelf database by user")
...
class User:
    ...
    def borrow(self, book: Book, shelf: Shelf):
        if self.role == "basic":
    ...
    def submit(self, book: Book, shelf: Shelf):
        if self.role == "basic":
    ...
    def reserve(self, book: Book, shelf: Shelf):
        if self.role == "basic":
    ...
    def add_books(self, book: Book, shelf: Shelf):
    ...
    def remove_books(self, book: Book, shelf: Shelf):
        if self.role == "librarian":
            shelf.books.remove(book)
            logging.info(f"Book {book.name} with ISBN {book.isbn} removed from shelf {self.genre} by user")
    ...
    def get_count(self, shelf: Shelf):
        print("No. of books in the given shelf are ==>")
        print(Shelf.count_books(shelf))

    def populate_(self, shelf: Shelf, excel_file):
        if self.role == "librarian"
            Shelf.populate_books(shelf, excel_file)
        else:
            print("Only librarian users can add books to the shelves")
